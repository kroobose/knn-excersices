function [A_est,res] ...
    = train_topkhingealph( K_tra, y_tra, lam, param )
%
% train_topkhingealph
%
...
qpsolver = 'cqkp'; 
if isfield( param, 'qpsolver' )
  qpsolver = param.qpsolver; 
end
...
% For CQKP
%
r_tmp1  = [1:ncts-1,ncts+1,ncts]; 
A_ineq2 = [A1_ineq;A3_ineq]; 
ncons_eq   = size(A_eq,1); ncons_ineq2 = size(A_ineq2,1); 
dvec_cqkp = [ones(1,ncts), 0, zeros(1,ncts+1)]'; 
avec_cqkp = zeros(2*ncts+2,1); 
bmat_cqkp = [ A_eq', A_ineq2(:,r_tmp1)'; ...
         zeros(ncts+1,ncons_eq), eye(ncons_ineq2) ]; 
rvec_cqkp = [ zeros( ncts+1, 1 ); 1 ]; 
lvec_cqkp = [ -ones(ncts-1,1); 0;  -1; zeros(ncts+1,1) ];  
uvec_cqkp = lvec_cqkp+1; 
cqkpmat = [avec_cqkp,bmat_cqkp,max( dvec_cqkp, 1e-6 ),lvec_cqkp,uvec_cqkp]; 
...
while ~finished,
  ...
  for i0_k=1:numel(r_con)
    ...
    switch(qpsolver)
      ...
      case 'cqkp'
        avec = [-f_qp; zeros(ncts+1,1)];         
        cqkpmat(:,1) = avec; 
        param_cqkp.verbose = 0; 
        param_cqkp.thres_vio = 1e-6*ncts; 
        param_cqkp.niters = 20; 
        [albemxi_ts,obj_ts,res] = kiwqp_solve( cqkpmat, rvec_cqkp, param_cqkp ); 
        albem = albemxi_ts(1:ncts+1); 
      otherwise
        tsassert(0);  
    end
    alphi = albem(1:ncts);     
