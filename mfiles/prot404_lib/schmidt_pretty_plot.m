function [output] = schmidt_pretty_plot(xData,yData,options)
...
[legendStr,plotTitle,plotXlabel,plotYlabel,lineWidth,colors,lineStyles,...
    ...
    schmidt_process_options(options,'legend',[],'title',[],'xlabel',[],'ylabel',[],...
...
fontsz_ts = 16; 
fontname = 'Times'; % 'AvantGarde'; 

set(gca,'FontName',fontname,'FontWeight','normal','FontSize',fontsz_ts);

if ~isempty(plotTitle)
    hTitle = title(plotTitle);
    set(hTitle,'FontName',fontname,'FontSize',fontsz_ts,'FontWeight','bold');
end

if ~isempty(plotXlabel) || ~isempty(plotYlabel)
    h1 = xlabel(plotXlabel);
    h2 = ylabel(plotYlabel);
    set([h1 h2],'FontName',fontname,'FontSize',fontsz_ts,'FontWeight','normal');
end
    ...
    if labelLines % Try to label the lines
        for i = 1:nLines
            ...
            hText(i) = text(x{i}(m),y{i}(m),...
                ...
                'FontSize',fontsz_ts,'FontWeight','bold');
        ...
        set(h,'FontSize',fontsz_ts-2,'FontWeight','normal');
        set(h,'Location','Best');
